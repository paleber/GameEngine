...
public class VectorTest {
    ...
    public void initWithXY() {
        v.initWithXY(5, 3);
        Assert.assertEquals(5, v.getX(), DELTA);
        Assert.assertEquals(3, v.getY(), DELTA);
    }

    @Test
    public void initWithAngleAndLength() {
        v.initWithAngleAndLength(0.75 * Math.PI, Math.sqrt(2));
        Assert.assertEquals(-1, v.getX(), DELTA);
        Assert.assertEquals(1, v.getY(), DELTA);

        v.initWithAngleAndLength(-5 * Math.PI, 2);
        Assert.assertEquals(-2, v.getX(), DELTA);
    ...
    public void getLength() {
        ...
        Assert.assertEquals(3, v.getLength(), DELTA);
    }

    @Test
    public void getAngle() {
        v.initWithAngleAndLength(-5 * Math.PI, 3);
        Assert.assertEquals(Math.PI, v.getAngle(), DELTA);
    }
    ...
    public void rotate() {
        ...
        Assert.assertEquals(0.75 * Math.PI, v.getAngle(), DELTA);
    }

    @Test
    public void toStringTest() {
        v.initWithXY(2, 3);
        Assert.assertEquals("<2.000|3.000>", v.toString());
    }
